...
// Parameter for BaBar Crystal
	...
	double light_yield_crs = 50000 * integration_frac * (1. / MeV);
	...
	// 400/402 = Panda Crystals Top/Bottom, 500/502 = FT crystals Top/Bottom
	if (sector == 400 || sector == 402 || sector == 500 || sector == 502) {
		...
		if (sector == 400 || sector == 402) light_yield_crs = 310 * integration_frac * (1. / MeV); //Panda Crystals LY
		if (sector == 500 || sector == 502) light_yield_crs = 310 * integration_frac * (1. / MeV); //FT Crystals LY
	}
...
//			timeF= timeF + (times[s] + dRight/veff) / nsteps;
	...
	dgtz["xch"] = xch;
	dgtz["ych"] = ych;
	// for BDX-MINI crystals ADCL_crs is in keV ! only valid if no attenuation !
	if (sector == 400 || sector == 402 || sector == 500 || sector == 502) {
	...
	dgtz["adcb"] = Etot_B_crs * 1000;  // deposited energy with Birks
	dgtz["adcf"] = Etot_noB_crs * 1000;
...
double JPOS_crs_HitProcess::BirksAttenuation(double destep, double stepl, int charge, double birks) {
...
double JPOS_crs_HitProcess::BirksAttenuation2(double destep, double stepl, int charge, double birks) {
...
//double JPOS_crs_HitProcess::WaveForm(double npe, double time)
double* JPOS_crs_HitProcess::WaveForm(double npe, double* time) {
	double c = exp(-2.);
//    double Time;
...
double* JPOS_crs_HitProcess::WaveFormPbwo(double npe, double* time_pbwo) {
	double c = exp(-2.);
	//    double Time;
	double t; // time in usec
	double WF;
