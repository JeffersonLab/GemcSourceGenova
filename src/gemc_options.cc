...
void goptions::setGoptions() {
	...
	optMap["JLABFLUX"].args = "no";
	optMap["JLABFLUX"].help = "JLabFlux measurement muon sampling";
	optMap["JLABFLUX"].help += "Use ID,hdet(cm),filename";
	optMap["JLABFLUX"].help += "default z-values: 2257.1cm (pipe-1), 2561.9cm (pipe-2). THESE ARE HARD_CODED";
	optMap["JLABFLUX"].help += "filename is the name of the ROOT file containing the two 3D histograms where the flux is saved";
	optMap["JLABFLUX"].help += "ID is 1 to sample from first pipe, 2 for the second";
	optMap["JLABFLUX"].help = "example1: -JLABFLUX=\"1,0,data.root\" to use first plane sampling, for a detector at h=0, with fname: data.root";
	optMap["JLABFLUX"].name = "JLabFlux simulations for pipe measurement, planeID, det. height, root-fname";
	optMap["JLABFLUX"].type = 1;
	optMap["JLABFLUX"].ctgr = "generator";

	optMap["JLABFLUX2"].args = "no";
	optMap["JLABFLUX2"].help = "JLabFlux measurement muon sampling NEW";
	optMap["JLABFLUX2"].help += "Use ID,minimum energy";
	optMap["JLABFLUX2"].help += "default z-values: 2257.1cm (pipe-1), 2561.9cm (pipe-2). THESE ARE HARD_CODED";
	optMap["JLABFLUX2"].help += "ID is 1 to sample from first pipe, 2 for the second";
	optMap["JLABFLUX2"].help = "example1: -JLABFLUX=\"1,100*MeV,\" to use first plane sampling and a minimum energy of 100 MeV";
	optMap["JLABFLUX2"].name = "JLabFlux simulations for pipe measurement, planeID, threshold";
	optMap["JLABFLUX2"].type = 1;
	optMap["JLABFLUX2"].ctgr = "generator";
