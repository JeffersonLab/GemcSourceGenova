...
MPrimaryGeneratorAction::MPrimaryGeneratorAction(goptions *opts) {
	...
	cArea = gemcOpt->optMap["COSMICAREA"].args;
	JLabFlux = gemcOpt->optMap["JLABFLUX"].args;
	JLabFlux2 = gemcOpt->optMap["JLABFLUX2"].args;
	...
	/*SOME HARDCODED VALUES FOR JLAB FLUX*/
	zPipe1 = 2257.1 * cm;
	zPipe2 = 2561.9 * cm;
	zOrigin = -290 * cm;
	JLabFluxDeltaZ = 41 * cm;
	//check surface
	JLabFluxDeltaX = 15 * cm; // 15cm
	JLabFluxDeltaY = 90 * cm; //90cm
	...
	if (input_gen == "gemc_internal") {
		...
		if (cosmics != "no") {
			...
			vector<string> cvalues = get_info(gemcOpt->optMap["JLABFLUX"].args, string(",\""));
			if (cvalues.size() != 3) cout << "  !!!  Warning:  JLABFLUX flag not set correctly. It should be 3 numbers: ID, height, root-fname " << JLabFlux << endl;
			JLabFluxID = atoi(cvalues[0].c_str());
			JLabFluxDetHeight = get_number(cvalues[1]);
			JLabFluxROOTfname = cvalues[2];

			if ((JLabFluxID < 1) || (JLabFluxID > 2)) {
				cout << JLabFluxID << endl;
				cout << " ERROR! ID in JLABFLUX option should be 1 or 2" << endl;
				exit(1);
			}
			JLabFluxROOTfile = new TFile(JLabFluxROOTfname.c_str());
			if (JLabFluxROOTfile->IsZombie()) {
				cout << " ERROR! ROOT FILE " << JLabFluxROOTfname << " not found" << endl;
				exit(1);
			}
			JLabFluxROOThisto = 0;
			JLabFluxROOThisto = (TH3D*) JLabFluxROOTfile->Get(Form("h3D_%i", JLabFluxID));
			if (JLabFluxROOThisto == 0) {
				cout << " ERROR! ROOT HISTO NOT FOUND IN" << JLabFluxROOTfile << endl;
				exit(1);
			}
			JLabFluxNORM = JLabFluxROOThisto->Integral("width");

			cout << endl << hd_msg << " Beam Type: JLabFlux" << endl;
			cout << hd_msg << "FLUX_ID: " << JLabFluxID << endl;
			cout << hd_msg << " Det-height: " << JLabFluxDetHeight / cm << " cm " << endl;
			cout << hd_msg << " root-fname: " << JLabFluxROOTfname << endl;
			cout << hd_msg << " hard-coded values of zPipe:" << endl;
			cout << hd_msg << "  zPipe1: " << zPipe1 << endl;
			cout << hd_msg << "  zPipe2: " << zPipe2 << endl;
			cout << hd_msg << " JLabFluxRootHisto name: " << JLabFluxROOThisto->GetName() << endl;
			cout << hd_msg << " JLabFluxNORM: " << JLabFluxNORM << endl;
		}
			...
			if (drdzOrdxdydz == 0) cout << " (radius, z-spread) = (" << dvr / cm << ", " << dvz / cm << ") cm, ";
				...
				cout << " (dvx, dvy, dvz) = (" << dvx / cm << ", " << dvy / cm << ", " << dvz / cm << ") cm, ";
...
// internal generator. Particle defined by command line
	if (input_gen == "gemc_internal") {
		...
		for (unsigned i = 0; i < 8; i++)
			headerUserDefined.push_back(0);

		// First option: cosmics, internal
		...
		//second option - specific for JLabFlux
		else if (JLabFlux != "no") {

			G4double phiPos, phiMom;
			TVector3 pos;
			TVector3 vertex;
			TVector3 dir;
			TVector3 mom;
			G4double R, E, Ctheta, x, y, z, x1, y1, t;
			int Nextr = 0;

			//z is fixed

			if (JLabFluxID == 1) z = zPipe1;
			else
				z = zPipe2;
			...
			while (Nextr < 1E7) {
				...
				//R=sqrt(R);
				/*Kinetic Energy in GeV, radius in cm*/
				E = E * GeV;
				R = R * cm;
				...
				//now, need to revert back to lab frame
				mom.RotateUz(dir.Unit());
				Ctheta = mom.CosTheta();
				phiMom = mom.Phi();

				/*Check hit position on the pipe - at z=+41 cm wrt the scoring plane*/
				t = JLabFluxDeltaZ / Ctheta;
				x1 = x + t * sqrt(1 - Ctheta * Ctheta) * cos(phiMom);
				y1 = y + t * sqrt(1 - Ctheta * Ctheta) * sin(phiMom);

				/*Check if this ray is within (more or less) the detector fiducial volume: +- 15 cm in x, +-30 cm in y centered at JLabFluxDetHeight wrt beamline*/
				if ((fabs(x1) < JLabFluxDeltaX) && (fabs(y1 - JLabFluxDetHeight) < JLabFluxDeltaY)) {
					break;
				}

			}

			//	h.Fill(phiPos,phiMom);
			//	h2.Fill(phiPos-phiMom,E/GeV);
			//	cout<<phiPos<<" "<<phiMom<<" "<<phiPos-phiMom<<" "<<E/GeV<<endl;
			if (Nextr > 999999) cout << " !!!! LOOPING IN JLABFLUX EXTRACTION !!! exceeded " << Nextr << " extractions !!!" << Nextr << endl;
			...
			particleGun->SetParticleMomentumDirection(beam_dir);
			particleGun->GeneratePrimaryVertex(anEvent);
			if (Ntot == 100 || Ntot == 1000 || Ntot == 10000 || Ntot == 100000 || Ntot == 500000 || Ntot == 1000000) {
				JLabFluxNORM = JLabFluxROOThisto->Integral("width");
				JLabFluxEFF = 1. * Ntot / Nsup;
				cout << hd_msg << " NORMALIZATION FOR MUONS GENERATION: " << JLabFluxNORM << endl;
				cout << hd_msg << "EFFICIENCY FOR MUONS GENERATION: " << JLabFluxEFF << endl;
				cout << hd_msg << "PRODUCT: " << JLabFluxNORM * JLabFluxEFF << " muons/EOT" << endl;
			}
		...
		// internal, no cosmic
			...
			if (gemcOpt->optMap["USE_GUI"].arg > 0) setBeam();

			// Primary Particle
			// if one uses the /gun/particle /gun/ion command then we
			// make sure not to overwrite those values
			if (Particle->GetParticleName() != "GenericIon") particleGun->SetParticleDefinition(Particle);
			...
			particleGun->GeneratePrimaryVertex(anEvent);
			if (GEN_VERBOSITY > 3) {
				cout << hd_msg << " Particle id=" << Particle->GetParticleName() << "  Vertex=" << beam_vrt / cm << "cm,  momentum=" << Mom / GeV << " GeV, theta=" << Theta / deg << " degrees,   phi=" << Phi / deg << " degrees" << endl;
				if (partPol > 0) cout << hd_msg << "   with polarization  angles: polar - " << polTheta / deg << " degrees, " << "azimuthal - " << polPhi / deg << " degrees ";
				cout << endl;
			}
		}
	}
...
// external generator: input file
	{
		...
		if ((gformat == "ROOT" || gformat == "root")) {
			headerUserDefined.clear();
			kinMu = 0;
			while (kinMu < JLabFluxMinE) {
				if (idxMu < JLabFluxROOTtree->GetEntries()) {
					JLabFluxROOTtree->GetEntry(idxMu++);

					if (idFluxMu != JLabFluxID) { //Check the pipe ID.
						kinMu = 0;
						continue;
					}
					...
					vyMu = vyMu * cm;
					vzMu = vzMu * cm;
					headerUserDefined.push_back(wMu);

					//need to change coord. system//TODO Better
					if (JLabFluxID == 1) vzMu = zPipe1;
					else
						vzMu = zPipe2;
					vzMu = vzMu - JLabFluxDeltaZ;
				} else {
					cout << hd_msg << " root file with muons ended. Return " << endl;
					return;
				}
			}
			// choosing charge of the muons
			...
			particleGun->SetParticleDefinition(Particle);
			particleGun->SetParticleEnergy(kinMu);
			particleGun->SetParticleMomentumDirection(beam_dir);
			particleGun->SetParticlePosition(G4ThreeVector(vxMu, vyMu, vzMu));
			particleGun->GeneratePrimaryVertex(anEvent);
		}
		...
		// type is 1 for particles in the detector
		else if ((gformat == "LUND" || gformat == "lund") && !gif.eof()) {
			headerUserDefined.clear();
			...
			for (int p = 0; p < NPART; p++) {
				if (stdhep_reader->daughter1(p) > 0) {
					...
					// vertex
					if (gemcOpt->optMap["STEER_BEAM"].arg == 0) {
						beam_vrt = G4ThreeVector(stdhep_reader->X(p) * cm, stdhep_reader->Y(p) * cm, stdhep_reader->Z(p) * cm);
					} else {
						// vertex smear and offset
						double VR = sqrt(G4UniformRand()) * dvr / mm;
						double PHI = 2.0 * pi * G4UniformRand();

						beam_vrt = G4ThreeVector(stdhep_reader->X(p) * cm + vx / mm + VR * cos(PHI), stdhep_reader->Y(p) * cm + vy / mm + VR * sin(PHI), stdhep_reader->Z(p) * cm + vz / mm + (2.0 * G4UniformRand() - 1.0) * dvz / mm);
					...
					particleGun->SetParticleTime(TWINDOW / 2);
					particleGun->GeneratePrimaryVertex(anEvent);
					if (GEN_VERBOSITY > 3) cout << hd_msg << " Particle Number:  " << p << ", id=" << stdhep_reader->pid(p) << "(" << Particle->GetParticleName() << ")" << "  Vertex=" << beam_vrt << "cm,  momentum=" << pmom / GeV << " GeV" << endl;
				}
			}
		}
	}
	...
	if (background_gen != "no") {
		...
		for (int p = 0; p < nparticles; p++) {
			...
			if (pindex == p + 1) {
				...
				particleGun->GeneratePrimaryVertex(anEvent);
				if (GEN_VERBOSITY > 3) cout << hd_msg << " Merged Particle Number:  " << p + 1 << ", id=" << pdef << " (" << Particle->GetParticleName() << ")" << "  Vertex=" << beam_vrt << "cm,  momentum=" << pmom / GeV << " GeV" << endl;
			} else if (pindex != p + 1) if (GEN_VERBOSITY > 3) cout << hd_msg << " Warning: file particle index " << tmp << " does not match read particle index " << p + 1 << endl;
		}
	}
...
// cout << PBUNCH << " " << NBUNCHES <<  " " << NBUNCHES*PBUNCH << endl;
	...
	if (PBUNCH > 0) {
		...
		for (int b = 0; b < NBUNCHES; b++) {
			...
			if (b == NBUNCHES - 1) particleGun->SetNumberOfParticles(PBUNCH + NREMAINING);
...
// Luminosity Particles2
	...
	if (GEN_VERBOSITY > 5) cout << " Generation done " << endl;
}
...
// vector of string - filled from the various option
	...
	if (input_gen == "gemc_internal") {
		if (cosmics == "no") {
			...
			if (!Particle) {
				...
				if (pname == "show_all") {
					for (int i = 0; i < particleTable->entries(); i++)
						cout << hd_msg << " g4 particle: " << particleTable->GetParticleName(i) << " pdg encoding: " << particleTable->GetParticle(i)->GetPDGEncoding() << endl;
				}
			...
			if (values.size() == 4) if (trimSpacesFromString(values[3]) == "flat") primaryFlat = 1;
			...
			if (csettings[0] == "default") {
				...
				cminp = get_number(csettings[1], 0);
				cmaxp = get_number(csettings[2], 0);
				...
				cminp = get_number(csettings[3], 0);
				cmaxp = get_number(csettings[4], 0);
	...
	} else if (input_gen.compare(0, 4, "ROOT") == 0 || input_gen.compare(0, 4, "root") == 0) {
		gformat.assign(input_gen, 0, input_gen.find(","));
		gfilename.assign(input_gen, input_gen.find(",") + 1, input_gen.size());

		vector<string> cvalues = get_info(gemcOpt->optMap["JLABFLUX2"].args, string(",\""));
		if (cvalues.size() != 2) cout << "  !!!  Warning:  JLABFLUX2 flag not set correctly. It should be 2 numbers: ID, minE " << JLabFlux << endl;
		JLabFluxID = atoi(cvalues[0].c_str());
		JLabFluxMinE = get_number(cvalues[1]);
		...
		cout << endl << hd_msg << " Beam Type: JLabFlux2" << endl;
		cout << hd_msg << "FLUX_ID: " << JLabFluxID << endl;
		cout << hd_msg << " Minumum energy: " << JLabFluxMinE / GeV << " GeV " << endl;
		cout << hd_msg << " hard-coded values of zPipe:" << endl;
		cout << hd_msg << "  zPipe1: " << zPipe1 << endl;
		cout << hd_msg << "  zPipe2: " << zPipe2 << endl;
		...
		JLabFluxROOTfile = new TFile(gfilename.c_str());
		JLabFluxROOTtree = (TTree*) JLabFluxROOTfile->Get("out");

		if (JLabFluxROOTtree == 0) {
			cout << hd_msg << " No TTre?? " << endl;
			exit(1);
		}
		JLabFluxROOTtree->SetMakeClass(1);

		//link variables
		JLabFluxROOTtree->SetBranchAddress("m_E", &kinMu);
		JLabFluxROOTtree->SetBranchAddress("m_cx", &cxMu);
		JLabFluxROOTtree->SetBranchAddress("m_cy", &cyMu);
		JLabFluxROOTtree->SetBranchAddress("m_cz", &czMu);

		JLabFluxROOTtree->SetBranchAddress("m_vx", &vxMu);			//vertice in cm
		JLabFluxROOTtree->SetBranchAddress("m_vy", &vyMu);
		JLabFluxROOTtree->SetBranchAddress("m_vz", &vzMu);

		JLabFluxROOTtree->SetBranchAddress("m_weight", &wMu);

		JLabFluxROOTtree->SetBranchAddress("m_fluxID", &idFluxMu);
		//...tMuon->SetBranchAddress("m_cx",&cxMu);

		cout << hd_msg << "ROOT: Opening  " << gformat << " file: " << trimSpacesFromString(gfilename).c_str() << endl;
		cout << hd_msg << " Entries: " << JLabFluxROOTtree->GetEntries() << endl;
	...
	else if (input_gen.compare(0, 6, "stdhep") == 0 || input_gen.compare(0, 6, "STDHEP") == 0 || input_gen.compare(0, 6, "StdHep") == 0 || input_gen.compare(0, 6, "StdHEP") == 0) {
		// StdHep is an (old like LUND) MC generator format in binary form.
		gformat.assign(input_gen, 0, input_gen.find(","));
		gfilename.assign(input_gen, input_gen.find(",") + 1, input_gen.size());
...
// Getting momentum spread from option value
	...
	if (values.size() == 4) if (trimSpacesFromString(values[3]) == "flat") lumiFlat = 1;
...
// Getting momentum spread from option value
	...
	if (values.size() == 4) if (trimSpacesFromString(values[3]) == "flat") lumi2Flat = 1;
...
void MPrimaryGeneratorAction::setParticleFromPars(int p, int pindex, int type, int pdef, double px, double py, double pz, double Vx, double Vy, double Vz, G4Event* anEvent) {
	...
	if (type == 1 && pindex == p + 1) {
		...
		if (GEN_VERBOSITY > 3) cout << hd_msg << " Particle Number:  " << p + 1 << ", id=" << pdef << " (" << Particle->GetParticleName() << ")" << "  Vertex=" << beam_vrt / cm << "cm,  momentum=" << pmom / GeV << " GeV" << endl;
		...
		if (GEN_VERBOSITY > 3) cout << hd_msg << " Warning: file particle index " << pindex << " does not match read particle index " << p + 1 << endl;
	}
}
