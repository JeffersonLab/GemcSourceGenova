...
PhysicsList::PhysicsList(goptions opts) :
	...
	for (unsigned i = 0; i < allEM.size(); i++) {
		...
		if (stripped != "") g4EMList.push_back(stripped);
...
void PhysicsList::ConstructParticle() {
	...
	G4Scalar::ScalarDefinition(scalarMass);
...
//	pmanager->AddDiscreteProcess(new G4Transportation);
	...
	//Scalar production
	vector<string> values;
	values = get_info(gemcOpt.optMap["SCALAR_BIAS"].args);
	G4double scalarBias = get_number(values[0]);

	G4MuonScalarProduction *myMuonScalarProduction;

	particle = G4MuonPlus::MuonPlus();
	pmanager = particle->GetProcessManager();
	myMuonScalarProduction = new G4MuonScalarProduction;
	myMuonScalarProduction->SetCrossSecFactor(scalarBias);
