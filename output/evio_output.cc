...
void evio_output::writeHeader(outputContainer *output, map<string, double> data, gBank bank) {
	...
	*headerBank << addVariable(HEADER_BANK_TAG, bank.getVarId("time"), "s", time);
	...
	for (map<string, double>::iterator it = data.begin(); it != data.end(); it++) {
		...
		if (bankId != -1) *headerBank << addVariable(HEADER_BANK_TAG, bankId, bank.getVarType(it->first), it->second);

		// user info
		if (bankId == -1) {
			if (it->first.find("userVarI") != string::npos) {
				int A = (int) it->second;
				uss << it->first << " " << A << endl;
			} else if (it->first.find("userVarL") != string::npos) {
				long A = (long) it->second;
				uss << it->first << " " << A << endl;
			} else if (it->first.find("userVarUL") != string::npos) {
				unsigned long A = (unsigned long) it->second;
				uss << it->first << " " << A << endl;
			} else if ((it->first.find("userVarF") != string::npos) || (it->first.find("userVarF") != string::npos)) {
		...
		if (it->first == "evn") evn = it->second;
	}
	*headerBank << addVariable(HEADER_BANK_TAG, bank.getVarId("user"), "s", uss.str());
...
// bank starts from 2 cause timestamp already there
	...
	/*A.C. with the following instructions I am sure to write AT LEAST 10 numbers, always*/
...
// collecting vectors from each hit into one big array
	...
	for (unsigned int nh = 0; nh < HO.size(); nh++) {
		...
		for (auto h : thisStep)
			allStep.push_back(h);
		for (auto h : thisCharge)
			allCharge.push_back(h);
		for (auto h : thisTime)
			allTime.push_back(h);
		for (auto h : thisID)
			allID.push_back(h);
...
// return right away if "d"
	if (type == "d") return evioDOMNode::createEvioDOMNode(tag, num, &value, 1);
...
// return right away if "d"
	if (type == "d") return evioDOMNode::createEvioDOMNode(tag, num, value);
